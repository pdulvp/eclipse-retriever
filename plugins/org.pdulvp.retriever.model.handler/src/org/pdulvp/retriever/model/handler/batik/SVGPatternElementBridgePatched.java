 ...
 */
package org.pdulvp.retriever.model.handler.batik;
 ...
 */
public class SVGPatternElementBridgePatched extends AnimatableGenericSVGBridge implements PaintBridge, ErrorConstants {

  /**
   * Constructs a new SVGPatternElementBridge.
   */
  public SVGPatternElementBridgePatched() {
  }
   ...
   *          the pattern element that defines a Paint
   * @param paintedElement
   *          the element referencing the paint
   * @param paintedNode
   *          the graphics node on which the Paint will be applied
   * @param opacity
   *          the opacity of the Paint to create
   */
  @Override
  public Paint createPaint(BridgeContext ctx, Element patternElement, Element paintedElement, GraphicsNode paintedNode,
      float opacity) {
    ...
    if (s.length() == 0) {
      ...
      contentCoordSystem = SVGUtilities.parseCoordinateSystem(patternElement, SVG_PATTERN_CONTENT_UNITS_ATTRIBUTE, s,
          ctx);
    }
    ...
    if (viewBoxStr.length() > 0) {
      ...
      AffineTransform preserveAspectRatioTransform = ViewBox.getPreserveAspectRatioTransform(patternElement, viewBoxStr,
          aspectRatioStr, w, h, ctx);
    ...
    if (opacity != 1) {
      ...
      filter = new ComponentTransferRable8Bit(filter, ConcreteComponentTransferFunction.getLinearTransfer(opacity, 0), // alpha
  ...
  /**
   * Returns the content of the specified pattern element. The content of the pattern can be specified as children of
   * the patternElement or children of one of its 'ancestor' (linked with the xlink:href attribute).
   *
   * @param patternElement
  ...
  protected static RootGraphicsNode extractPatternContent(Element patternElement, BridgeContext ctx) {
    ...
    for (;;) {
      ...
      if (contains(refs, purl)) {
        throw new BridgeException(ctx, patternElement, ERR_XLINK_HREF_CIRCULAR_DEPENDENCIES, new Object[] { uri });
      }
      refs.add(purl);
